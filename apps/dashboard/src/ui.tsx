...
// escapeForHtml first so `<`, `>`, `&` in program output stay literal; ansiToHtml then
// turns the (256-color) ANSI codes into inline-styled <span>s — e.g. lolcat / cowsay art.
export function AnsiText({ text, className }: { text: string; className?: string }) {
  return (
    <span
      className={className}
      dangerouslySetInnerHTML={{ __html: Anser.ansiToHtml(Anser.escapeForHtml(text)) }}
    />
  );
}
